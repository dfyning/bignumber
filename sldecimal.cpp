...
void sldecimal::setprecision(unsigned long long precision) { // 精度调整
...
void sldecimal::currectsetting(const std::string & strnum) {
	unsigned long long positionofpoint = strnum.find('.'); // 小数点所在位置
	if (positionofpoint == std::string::npos) { // 无小数部分按整形处理
		this->integer = hugeinteger(strnum);
		this->pointer = 0;
	}
	else {
		...
		if ((*strindex == '0') && *(strindex + 1) == 't') {  // 十进制非常麻烦， 需要另外处理
			...
			sldecimal answer = hugeinteger(std::string(beg, beg + positionofpoint));
			hugeinteger ten("+0xa");
		...
		this->integer = hugeinteger(std::string(beg, iterofpoint) + newstrfordecimal);
...
sldecimal::sldecimal() {
	this->integer = hugeinteger();
	this->pointer = 0;
}

sldecimal::sldecimal(const std::string & strnum, unsigned long long numofdecimal) {
...
sldecimal::sldecimal(const hugeinteger& integer) {
	this->integer = integer;
	this->pointer = 0;
	this->precision = sldecimal_precision;
}

sldecimal::sldecimal(const hugeinteger& integer, unsigned long long numofdecimal) {
	this->precision = numofdecimal;
	this->integer = integer;
	this->pointer = numofdecimal;
	this->precision = numofdecimal;
...
sldecimal::sldecimal(const sldecimal& slnum, unsigned long long numofdecimal) {
...
hugeinteger sldecimal::getinteger() const{
...
bool operator>(const sldecimal & number, const sldecimal & number1) {
	...
	long long longerdecimal = max(number.pointer, number1.pointer); // 对齐 此时无需考虑正负
	hugeinteger copy, copy1;
...
bool operator==(const sldecimal & number, const hugeinteger & number1) {
	if (number == sldecimal(number1)) return true;
	return false;
}

bool operator==(const hugeinteger & number, const sldecimal & number1) {
...
sldecimal operator+(const sldecimal & number, const sldecimal & number1) {
	...
	if (number.pointer < number1.pointer) {
		answer.integer = hugeinteger((number.integer << (number1.pointer - number.pointer)) + number1.integer);
		answer.pointer = number1.pointer;
	}
	else if (number.pointer > number1.pointer) {
		hugeinteger copy(number1.integer);
		answer.integer = hugeinteger((copy << (number.pointer - number1.pointer)) + number.integer);
...
sldecimal operator-(const sldecimal & number, const sldecimal & number1) {
	...
	if (number.pointer < number1.pointer) {
		...
		answer.integer = hugeinteger(number.integer - (number1.integer << (number.pointer - number1.pointer)));
		answer.pointer = number.pointer;
	}
	else {
		answer.integer = (number.integer - number1.integer);
...
sldecimal operator/(const sldecimal & number, const sldecimal & number1) {
	...
	answer.integer = number.integer / number1.integer;
	answer.pointer = number.pointer - number1.pointer;
	hugeinteger remainder = number.integer % number1.integer;
...
sldecimal operator+(const hugeinteger & number, const sldecimal & number1) {
	return sldecimal(number) + number1;
}

sldecimal operator+(const sldecimal & number, const hugeinteger & number1) {
...
sldecimal sldecimal::operator<<(const unsigned long long leftlen) const {
	sldecimal answer(*this);
	answer.pointer = answer.pointer - leftlen;
	return answer;
}

sldecimal sldecimal::operator>>(const unsigned long long rightlen) const {
	sldecimal answer(*this);
...
void sldecimal::delete0inbottom() {
	unsigned long long numsof0;
	numsof0 = this->integer.delete0inbottom();
	this->pointer -= numsof0;
}

std::string sldecimal::output(int index) {
